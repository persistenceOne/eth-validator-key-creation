...
class Credential:
    ...
    def save_signing_keystore(self, password: str, folder: str) -> [str, str]:
        keystore = self.signing_keystore(password)
        keystore_file_name = 'keystore-%s-%i.json' % (keystore.path.replace('/', '_'), time.time())
        filefolder = os.path.join(folder, keystore_file_name)
...
class CredentialList:
    ...
    def export_deposit_data_json(self, folder: str) -> [str, str]:
